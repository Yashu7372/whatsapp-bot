...
import com.whatsappbot.document.DocumentAuthorizationService;
...
public class ProjectDeliveryService {
    ...
    public PortfolioView portfolio(UUID tenantId, UUID userId) {
        ...
        List<ProjectCardView> projects = projectService.list(tenantId, userId, "ACTIVE").stream()
                .map(p -> toProjectCard(tenantId, actor, p))
                .toList();
        BigDecimal totalContract = projects.stream().map(ProjectCardView::visibleContractValue)
        ...
        boolean anyCommercial = projects.stream().anyMatch(p -> !"NONE".equals(p.commercialVisibility()));
        return new PortfolioView(repository.accountName(tenantId), projects.size(), totalContract,
                totalActual, openItems, blockedItems, overdueDocuments, anyCommercial, projects);
    }
        ...
        List<StageView> stages = repository.stages(tenantId, projectId).stream()
                .map(stage -> {
                    List<WorkPackageView> packages = repository.packages(tenantId, projectId, stage.id()).stream()
                            .map(pkg -> toPackage(tenantId, userId, projectId, pkg, scope))
                            .toList();
                ...
                .map(item -> {
                    ...
                    List<WorkDocumentView> documents = repository.documents(tenantId, item.id()).stream()
                            ...
                            .map(d -> new WorkDocumentView(d.id(), d.documentCode(), d.title(), d.docType(),
                                    d.status(), d.revisionCode(), d.reviewOutcome(), d.dueAt(),
                                    commercial ? d.approvedValue() : BigDecimal.ZERO))
                            .toList();
                    return new WorkItemView(item.id(), item.itemCode(), item.name(), item.workType(), item.status(),
    ...
    private boolean canViewDocument(UUID tenantId, UUID userId, UUID documentId) {
        try {
            ...
            if (denied.getStatusCode().value() == 403 || denied.getStatusCode().value() == 404) return false;
            throw denied;
        }
    }
    ...
    public record PortfolioView(String accountName, int activeProjects, BigDecimal totalContractValue,
                                ...
                                int overdueDocuments, boolean commercialVisible, List<ProjectCardView> projects) {}
    public record ProjectCardView(UUID id, String projectCode, String name, String status, String currency,
                                  BigDecimal visibleContractValue, String commercialVisibility,
                                  LocalDate startDate, LocalDate endDate, BigDecimal progressPercent,
                                  BigDecimal actualCost, int openWorkItems, int blockedWorkItems,
                                  int overdueDocuments, int pendingApprovals, int participantCount,
                                  int stageCount, int completedStages) {}
    public record ProjectDetailView(UUID id, String projectCode, String name, String description, String status,
                                    String currency, BigDecimal contractValue, String commercialVisibility,
                                    LocalDate startDate, LocalDate endDate, DeliveryKpis kpis,
                                    List<ParticipantView> participants, List<StageView> stages) {}
