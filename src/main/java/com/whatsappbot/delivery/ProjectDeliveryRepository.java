...
class ProjectDeliveryRepository {
    ...
    ProjectMetrics metrics(UUID tenantId, UUID projectId) {
        BigDecimal progress = decimal("select coalesce(avg(progress_percent),0) from work_items where tenant_id=? and project_id=?",
                tenantId, projectId);
        BigDecimal actual = decimal("select coalesce(sum(amount),0) from actual_cost_entries where tenant_id=? and project_id=?",
                tenantId, projectId);
        BigDecimal hours = decimal("select coalesce(sum(hours),0) from timesheets where tenant_id=? and project_id=?",
                tenantId, projectId);
        int open = count("select count(*) from work_items where tenant_id=? and project_id=? and status not in ('COMPLETED','CLOSED','CANCELLED')",
                tenantId, projectId);
        int blocked = count("select count(*) from work_items where tenant_id=? and project_id=? and status='BLOCKED'",
                tenantId, projectId);
        int overdue = count("select count(*) from documents where tenant_id=? and project_id=? and due_at<now() and status not in ('APPROVED','PUBLISHED','ARCHIVED')",
                tenantId, projectId);
        int pending = count("""
                select count(*) from document_approvals a join documents d on d.id=a.document_id
                 where a.tenant_id=? and d.project_id=? and a.status='PENDING'
                "", tenantId, projectId);
        int participants = count("select count(*) from project_participants where tenant_id=? and project_id=? and active=true",
                tenantId, projectId);
        int stages = count("select count(*) from project_stages where tenant_id=? and project_id=?",
                tenantId, projectId);
        int completedStages = count("select count(*) from project_stages where tenant_id=? and project_id=? and status='COMPLETED'",
                tenantId, projectId);
        return new ProjectMetrics(progress, actual, open, blocked, overdue, pending, participants, stages,
                completedStages, hours);
    }

    List<ParticipantRow> participants(UUID tenantId, UUID projectId) {
        return jdbc.query("""
                select p.id,p.organization_id,o.name,o.org_code,p.party_role,p.parent_participant_id,
                ...
                "", (rs, n) -> new ParticipantRow(rs.getObject(1, UUID.class), rs.getObject(2, UUID.class),
                rs.getString(3), rs.getString(4), rs.getString(5), rs.getObject(6, UUID.class), rs.getInt(7)),
                tenantId, projectId);
    }

    List<StageRow> stages(UUID tenantId, UUID projectId) {
        return jdbc.query("""
                select id,stage_code,name,stage_type,sequence_no,status,progress_percent,
                       planned_start,planned_end,actual_start,actual_end
                  from project_stages
                 where tenant_id=? and project_id=?
                 order by sequence_no
                "", (rs, n) -> new StageRow(rs.getObject(1, UUID.class), rs.getString(2), rs.getString(3),
                rs.getString(4), rs.getInt(5), rs.getString(6), rs.getBigDecimal(7), date(rs, 8), date(rs, 9),
                date(rs, 10), date(rs, 11)), tenantId, projectId);
    }

    List<PackageRow> packages(UUID tenantId, UUID projectId, UUID stageId) {
        return jdbc.query("""
                select id,package_code,name,discipline,status
                  from work_packages
                 where tenant_id=? and project_id=? and stage_id=?
                 order by sort_order,package_code
                "", (rs, n) -> new PackageRow(rs.getObject(1, UUID.class), rs.getString(2), rs.getString(3),
    ...
    List<WorkItemRow> workItems(UUID tenantId, UUID projectId, UUID packageId) {
        return jdbc.query("""
                select w.id,w.item_code,w.name,w.work_type,w.status,w.priority,w.progress_percent,
                 ...
                 where w.tenant_id=? and w.project_id=? and w.package_id=?
                 order by w.sort_order,w.item_code
                "", (rs, n) -> new WorkItemRow(rs.getObject(1, UUID.class), rs.getString(2), rs.getString(3),
                rs.getString(4), rs.getString(5), rs.getString(6), rs.getBigDecimal(7),
                rs.getObject(8, UUID.class), rs.getString(9), rs.getObject(10, UUID.class), nz(rs.getBigDecimal(11)),
                nz(rs.getBigDecimal(12)), nz(rs.getBigDecimal(13)), rs.getInt(14), rs.getInt(15), rs.getString(16),
                date(rs, 17), date(rs, 18), date(rs, 19), date(rs, 20)), tenantId, projectId, packageId);
    }

    List<AssignmentRow> assignments(UUID tenantId, UUID workItemId) {
        return jdbc.query("""
                select a.user_id,u.full_name,u.job_title,u.department,u.role,o.name,a.responsibility
                  from work_item_assignments a
                  join tenant_users u on u.id=a.user_id
                  join organizations o on o.id=a.organization_id
                 where a.tenant_id=? and a.work_item_id=? and a.active=true and u.active=true
                 order by a.sort_order,u.full_name
                "", (rs, n) -> new AssignmentRow(rs.getObject(1, UUID.class), rs.getString(2), rs.getString(3),
    ...
    List<DocumentRow> documents(UUID tenantId, UUID workItemId) {
        return jdbc.query("""
                select id,document_code,title,doc_type,status,current_revision_code,review_outcome,due_at,approved_value
                  from documents
                 where tenant_id=? and work_item_id=?
                 order by updated_at desc
                 limit 25
                "", (rs, n) -> new DocumentRow(rs.getObject(1, UUID.class), rs.getString(2), rs.getString(3),
                rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), timestamp(rs, 8),
                nz(rs.getBigDecimal(9))), tenantId, workItemId);
    }
    ...
    record ProjectMetrics(BigDecimal progressPercent, BigDecimal actualCost, int openWorkItems,
                          int blockedWorkItems, int overdueDocuments, int pendingApprovals,
                          int participantCount, int stageCount, int completedStages, BigDecimal totalHours) {}
    ...
    record StageRow(UUID id, String stageCode, String name, String stageType, int sequenceNo, String status,
                    BigDecimal progressPercent, LocalDate plannedStart, LocalDate plannedEnd,
