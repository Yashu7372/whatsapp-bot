...
public class DocumentDeliveryService {
    ...
    public IssuedRevision issueCurrentRevision(UUID tenantId,UUID userId,UUID documentId,String purpose){
        ...
        if("ISSUED".equals(revision.issueStatus()))
            throw new ResponseStatusException(HttpStatus.CONFLICT,"Current revision is already issued");
    ...
    public UUID createTransmittal(UUID tenantId,UUID userId,UUID projectId,CreateTransmittalRequest req){
        ...
        if(sender==null || !repository.activeProjectOrganization(tenantId,projectId,sender))
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Sender organization is not active on this project");
        if(req.transmittalNo()==null || req.transmittalNo().isBlank())
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"transmittalNo is required");
    ...
    public void addItem(UUID tenantId,UUID userId,UUID transmittalId,UUID documentId,UUID versionId){
        ...
        if(!repository.revisionBelongsToProject(tenantId,tx.projectId(),documentId,versionId))
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Transmittals may contain only issued revisions from the same project");
    ...
    public void addRecipient(UUID tenantId,UUID userId,UUID transmittalId,UUID recipientOrganizationId){
        DocumentDeliveryRepository.TransmittalOwner tx=requireEditableTransmittal(tenantId,userId,transmittalId,ProjectPermission.TRANSMITTAL_CREATE);
        if(recipientOrganizationId==null || recipientOrganizationId.equals(tx.senderOrganizationId()))
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Recipient must be a different project organization");
        if(!repository.activeProjectOrganization(tenantId,tx.projectId(),recipientOrganizationId))
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Recipient organization is not active on this project");
    ...
    public void issueTransmittal(UUID tenantId,UUID userId,UUID transmittalId){
        DocumentDeliveryRepository.TransmittalOwner tx=requireEditableTransmittal(tenantId,userId,transmittalId,ProjectPermission.TRANSMITTAL_ISSUE);
    ...
    public void acknowledge(UUID tenantId,UUID userId,UUID transmittalId){
        ...
        if(d.organizationId()==null)
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Tenant administrators must acknowledge through a recipient organization user");
        if(repository.acknowledge(tenantId,transmittalId,d.organizationId(),userId)==0)
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,"Your organization is not an unacknowledged recipient of this transmittal");
    }
    ...
    private DocumentDeliveryRepository.TransmittalOwner requireEditableTransmittal(UUID tenantId,UUID userId,UUID transmittalId,ProjectPermission permission){
        ...
        if(!accessService.isTenantAdministrator(d.actor()) && !tx.senderOrganizationId().equals(d.organizationId()))
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,"Only the sender organization can change this transmittal");
        return tx;
    }

    private static String normalizePurpose(String purpose){
        String p=purpose==null?"FOR_INFORMATION":purpose.trim().toUpperCase();
        if(!PURPOSES.contains(p)) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Unsupported issue purpose: "+p);
        return p;
    }
