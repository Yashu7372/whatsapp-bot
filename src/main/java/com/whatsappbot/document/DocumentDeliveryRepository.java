...
class DocumentDeliveryRepository {
    ...
    RevisionRef currentRevision(UUID tenantId, UUID documentId) {
        return jdbc.query("""
                select d.project_id,d.originator_org_id,d.current_version,v.id,v.revision_code,v.issue_status
                  from documents d join document_versions v on v.document_id=d.id and v.version_num=d.current_version
                 where d.tenant_id=? and d.id=?
                """,rs->rs.next()?new RevisionRef(rs.getObject(1,UUID.class),rs.getObject(2,UUID.class),rs.getInt(3),rs.getObject(4,UUID.class),rs.getString(5),rs.getString(6)):null,tenantId,documentId);
    }

    List<IssuedRevisionView> issuedRevisions(UUID tenantId,UUID projectId,UUID organizationId){
        String scope=organizationId==null?"":" and d.originator_org_id=?";
        Object[] args=organizationId==null?new Object[]{tenantId,projectId}:new Object[]{tenantId,projectId,organizationId};
        return jdbc.query("""
            select d.id,d.document_code,d.title,d.doc_type,v.id,v.version_num,v.revision_code,v.issue_purpose,v.issued_at
              from documents d join document_versions v on v.document_id=d.id
             where d.tenant_id=? and d.project_id=? and v.issue_status='ISSUED'
            """+scope+" order by d.document_code,v.version_num desc",(rs,n)->new IssuedRevisionView(
                rs.getObject(1,UUID.class),rs.getString(2),rs.getString(3),rs.getString(4),rs.getObject(5,UUID.class),
                rs.getInt(6),rs.getString(7),rs.getString(8),rs.getTimestamp(9)==null?null:rs.getTimestamp(9).toLocalDateTime()),args);
    }

    void issueCurrentRevision(UUID tenantId, UUID documentId, UUID versionId, UUID actorId, String purpose) {
        jdbc.update("update document_versions set issue_status='SUPERSEDED' where tenant_id=? and document_id=? and issue_status='ISSUED' and id<>?",tenantId,documentId,versionId);
        jdbc.update("update document_versions set issue_status='ISSUED',issue_purpose=?,issued_at=now(),issued_by=? where tenant_id=? and id=?",purpose,actorId,tenantId,versionId);
        jdbc.update("""
                update documents d set status='PUBLISHED',issue_purpose=?,current_revision_code=v.revision_code,
                  ...
                  from document_versions v where d.tenant_id=? and d.id=? and v.id=?
    ...
    UUID createTransmittal(UUID id,UUID tenantId,UUID projectId,UUID senderOrg,String number,String purpose,String subject,String message,UUID actorId){
        jdbc.update("insert into document_transmittals(id,tenant_id,project_id,transmittal_no,sender_organization_id,purpose,subject,message,created_by) values(?,?,?,?,?,?,?,?,?)",id,tenantId,projectId,number,senderOrg,purpose,subject,message,actorId);return id;
    }
    ...
    List<TransmittalView> list(UUID tenantId,UUID projectId,UUID organizationId){
        ...
        return jdbc.query("""
                ...
                """+scope+" order by t.created_at desc",(rs,n)->new TransmittalView(rs.getObject(1,UUID.class),rs.getString(2),rs.getObject(3,UUID.class),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getTimestamp(8)==null?null:rs.getTimestamp(8).toLocalDateTime(),rs.getTimestamp(9).toLocalDateTime(),rs.getInt(10),rs.getInt(11)),args);
    }
    ...
    record IssuedRevisionView(UUID documentId,String documentCode,String title,String docType,UUID versionId,int versionNum,String revisionCode,String purpose,LocalDateTime issuedAt){}
    record TransmittalOwner(UUID projectId,UUID senderOrganizationId,String status){}
    record TransmittalView(UUID id,String transmittalNo,UUID senderOrganizationId,String senderOrganizationName,String purpose,String subject,String status,LocalDateTime issuedAt,LocalDateTime createdAt,int itemCount,int recipientCount){}
}
