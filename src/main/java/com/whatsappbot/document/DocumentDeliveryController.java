...
public class DocumentDeliveryController {
    ...
    public ResponseEntity<Void> addItem(@AuthenticationPrincipal Claims claims,@PathVariable UUID transmittalId,
                                        @RequestBody AddItemRequest request){
        service.addItem(tenantId(claims),userId(claims),transmittalId,request.documentId(),request.versionId());
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/transmittals/{transmittalId}/recipients/{organizationId}")
    public ResponseEntity<Void> addRecipient(@AuthenticationPrincipal Claims claims,@PathVariable UUID transmittalId,
                                             @PathVariable UUID organizationId){
        service.addRecipient(tenantId(claims),userId(claims),transmittalId,organizationId);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/transmittals/{transmittalId}/issue")
    public ResponseEntity<Void> issue(@AuthenticationPrincipal Claims claims,@PathVariable UUID transmittalId){
        service.issueTransmittal(tenantId(claims),userId(claims),transmittalId);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/transmittals/{transmittalId}/acknowledge")
    public ResponseEntity<Void> acknowledge(@AuthenticationPrincipal Claims claims,@PathVariable UUID transmittalId){
        service.acknowledge(tenantId(claims),userId(claims),transmittalId);
        return ResponseEntity.noContent().build();
    }
