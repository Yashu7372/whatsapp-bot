...
class ApprovalWorklistRepository {
    ...
    List<Row> pending(UUID tenantId){
        return jdbc.query("""
            select s.id,a.id,a.document_id,d.project_id,d.document_code,d.title,
             ...
             order by coalesce(s.due_at,'9999-12-31'::timestamp),a.started_at
            """,(rs,n)->new Row(
    ...
    List<Row> overdue(UUID tenantId){
        return pending(tenantId).stream().filter(r->r.dueAt()!=null&&r.dueAt().isBefore(LocalDateTime.now())&&r.escalatedAt()==null).toList();
    }
    ...
    void enqueueEscalation(UUID tenantId,Row r){
        jdbc.update("""
            insert into workflow_notification_outbox(
                tenant_id,project_id,document_id,approval_id,approval_step_id,event_type,
                target_organization_id,target_party_role,payload)
            values(?,?,?,?,?,'APPROVAL_OVERDUE',?,?,jsonb_build_object(
                'documentCode',?,'title',?,'stepName',?,'authority',?,'dueAt',?))
            on conflict do nothing
            """,tenantId,r.projectId(),r.documentId(),r.approvalId(),r.stepId(),
                r.assignmentOrganizationId(),r.assignmentPartyRole(),r.documentCode(),r.title(),r.stepName(),r.authorityType(),
                r.dueAt()!=null?r.dueAt().toString():null);
    }

    private static LocalDateTime ts(java.sql.Timestamp t){return t==null?null:t.toLocalDateTime();}
    record Row(UUID stepId,UUID approvalId,UUID documentId,UUID projectId,String documentCode,String title,
               int stepIndex,String stepName,String authorityType,String assignmentType,UUID assignmentOrganizationId,
               String assignmentPartyRole,String reviewerEmail,boolean required,String parallelGroup,
               LocalDateTime dueAt,LocalDateTime escalatedAt,LocalDateTime startedAt){}
}
